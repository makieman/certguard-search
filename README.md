...
- **Verifier (Foundry IQ)**: Performs RAG against the vector index (Azure AI Search) to locate documentation alignment, returning cited verdicts.
...
- **Judge**: Synthesizes the debate logs and outputs either `ready`, `not_ready`, or `abstain` along with a confidence rating. If confidence is below `0.55`, it abstains.
- **Explainer (Work IQ)**: Takes scheduling constraints from `work_signals.json` and creates a customized study calendar.
...
1. **Foundry IQ (Knowledge Base)**: Managed via **Azure AI Search** (Free Tier, South Africa North). It index 6 synthetic markdown guides:
   - `az900_guide.md` (Fundamentals)
   - `az204_guide.md` (Developer)
   - `az305_guide.md` (Architect)
   - `az400_guide.md` (DevOps)
   - `team_report.md` (Organizational performance metrics)
   - `workload_report.md` (Typical cloud workload demands)
2. **Fabric IQ (Semantic Cert Model)**: Stored in `data/certs.json`, defining the schema of 5 Azure certifications (AZ-900, AZ-204, AZ-400, AZ-305, DP-203), mapping domains to prerequisite skills and thresholds.
3. **Work IQ (Activity & Scheduling Signals)**: Stored in `data/work_signals.json`, tracking active meeting hours, dedicated focus hours, and preferred learning slots per employee.
...
- **Frontend**: **React** + **Vite** web app with Study Plan and Agent Debate Trace views.
...
│   ├── verifier.py            # RAG semantic verification against Azure AI Search
...
│   └── llm_client.py          # Unified LLM provider & fallback manager
├── api/
